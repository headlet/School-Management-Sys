      ...
      @foreach($class as $classes)
        <div id="class- $classes->id " class="flex flex-col justify-between items-center w-full min-h-[100px] sm:min-h-[150px] md:h-10 border border-gray-400 rounded-lg sm:rounded-xl shadow-lg p-2 sm:p-3 bg-white hover:shadow-xl transition">
            <div class="flex flex-col items-center w-full flex-grow justify-center">
                <span class="text-[10px] sm:text-xs md:text-sm text-gray-500 truncate w-full text-center px-1">Class:{{$classes->Class}} </span>
                <p class="font-semibold text-xs sm:text-sm md:text-base text-gray-800 truncate w-full text-center px-1 mt-0.5">Section: {{$classes->Section}} </p>
            </div>

            <!-- Actions -->
            <div class="flex gap-1.5 sm:gap-2 md:gap-3 mt-1.5 sm:mt-2 md:mt-3">
                <a href="route('viewstd', $classes->id)" class="flex items-center justify-center w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 bg-blue-500 text-white rounded-full hover:bg-blue-600 transition">
